...
def score(df_cand_subject, conditions):
    ...
    #オンデマのスコア
    if conditions.is_ondemand_priority and (df["授業方法区分"] == "オンデマ").any():
        df["score"] += 80 
    print(df[["科目名", "score"]])
    df["学年"] = df["学年"].astype(str)  # `学年` を文字列に変換

    df.loc[
        (df["学部"] != conditions.faculty) |
        (df["学科"] != conditions.department) |
...
if __name__ == '__main__':
    cond = req("基幹理工学部", "応用数理学科", "3", "秋学期", True,1,2,False,3,2,1)
